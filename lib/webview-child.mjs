...
const { CW_URL, CW_TITLE, CW_WIDTH, CW_HEIGHT, CW_ICON_PATH, CW_THEME } = process.env;

// Map our string theme contract ("light"|"dark"|"system") to the runtime enum.
// Affects the native window chrome (Win11 titlebar dark/light, native form
// control colors). Not changeable at runtime — see lib/copilot-webview.js.
const themeMap = { light: Theme.Light, dark: Theme.Dark, system: Theme.System };
const initialTheme = themeMap[(CW_THEME ?? "system").toLowerCase()] ?? Theme.System;

const app = new Application();
const win = app.createBrowserWindow({ title: CW_TITLE, width: +CW_WIDTH, height: +CW_HEIGHT, theme: initialTheme });
...
// IPC handler. The page's document.title changes are NOT reflected to the OS
// window title bar by webview2/wry — we have to call win.setTitle() ourselves.
// Since the Node event loop is blocked by app.run(), the page can't reach the
// extension parent fast enough via WebSocket → eval → setTitle (the parent
// would need a way to call into the child). Instead the page calls
// window.ipc.postMessage(JSON.stringify({type: "setTitle", value})) which fires
// this handler INSIDE the event-loop callback context, where calling
// win.setTitle() is safe.
