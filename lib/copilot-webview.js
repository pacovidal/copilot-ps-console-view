...
//   new CopilotWebview({ extensionName, contentDir, callbacks?, title?, width?, height?, iconPath?, theme? })
...
//         .setTheme(mode)        → set the native window theme used on the
//                                  *next* open. Accepts "light" | "dark" |
//                                  "system". Does NOT update an already-open
//                                  window — the runtime's event loop is
//                                  blocked once the window is showing.
...
async function showWebview({ dir, title = "Copilot Webview", width = 900, height = 700, iconPath = null, theme = "system", callbacks = {} } = {}) {
    ...
    const childEnv = { ...process.env, CW_URL: url, CW_TITLE: title, CW_WIDTH: String(width), CW_HEIGHT: String(height), CW_THEME: String(theme) };
    if (userDataDir) childEnv.WEBVIEW2_USER_DATA_FOLDER = userDataDir;
    if (iconPath) childEnv.CW_ICON_PATH = isAbsolute(iconPath) ? iconPath : resolve(process.cwd(), iconPath);
...
export class CopilotWebview {
    constructor({ extensionName, contentDir, callbacks = {}, title, width, height, iconPath, theme = "system" } = {}) {
        ...
        this.width = width;
        this.height = height;
        this.iconPath = iconPath ? (isAbsolute(iconPath) ? iconPath : resolve(this.contentDir, iconPath)) : null;
    ...
    setTheme(mode) { this.theme = normalizeTheme(mode); }
